...
const HUB_KEY = "completeInteriors";
...
function liveChildPath(slug: string) {
  const base = IA_HUB_PATHS[HUB_KEY] || "/complete-interiors";
  return `${base}/${slug}`;
}
...
export default function VarsoviaCompleteInteriorsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(varsoviaErrorMessage(err, "Failed to load complete interiors pages"));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const persistChildren = async (nextChildren: IaChildRow[]) => {
    ...
    try {
      ...
      toast.success("Complete Interiors sub-page saved");
    } catch (err) {
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Search complete interiors sub-pages…"
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-[#E2E5EA] bg-white p-12 text-center">
            ...
            <p className="text-sm text-[#6B7280]">No complete interiors sub-pages yet</p>
          </div>
        ) : (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-lg font-semibold text-[#1A2332]">
                {modal === "create" ? "Add Complete Interiors sub-page" : "Edit Complete Interiors sub-page"}
              </h2>
